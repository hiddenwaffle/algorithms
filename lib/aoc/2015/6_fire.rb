...
class AoC::AoC_2015
  ...
  def day_6
    ...
    puts lights.flatten.filter { |b| b }.count
  end
  ...
  def switch(lights, command, row1, col1, row2, col2)
    ...
    row_times.times do |row|
      col_times.times do |col|
        case command
        when 'turn on'
          lights[row + row1][col + col1] = true
        when 'turn off'
          lights[row + row1][col + col1] = false
        when 'toggle'
          lights[row + row1][col + col1] = !lights[row + row1][col + col1]
        end
      end
    end
  end
